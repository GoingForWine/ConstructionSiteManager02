...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        /// Same but using the other rotation method
        public Quaternion GetBackwardRotationAtDistance(float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            float t = dst / length;
            return GetBackwardRotation(t, endOfPathInstruction);
        }
        ...
        /// Same but -direction in the end, to look ahead when moving backwards. 
        public Quaternion GetBackwardRotation(float t, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            var data = CalculatePercentOnPathData(t, endOfPathInstruction);
            Vector3 direction = Vector3.Lerp(localTangents[data.previousIndex], localTangents[data.nextIndex], data.percentBetweenIndices);
            Vector3 normal = Vector3.Lerp(localNormals[data.previousIndex], localNormals[data.nextIndex], data.percentBetweenIndices);
